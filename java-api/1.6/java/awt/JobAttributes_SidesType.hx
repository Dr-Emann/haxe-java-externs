package java.awt;

import java.awt.AttributeValue;

/** @REF http://docs.oracle.com/javase/6/docs/api/java/awt/JobAttributes.SidesType.html */
@:native("java.awt.JobAttributes.SidesType") @:final
extern class JobAttributes_SidesType extends AttributeValue
{

	/** @REF http://docs.oracle.com/javase/6/docs/api/java/awt/JobAttributes.SidesType.html#hashCode() */
	override public function hashCode():Int;

	/** @REF http://docs.oracle.com/javase/6/docs/api/java/awt/JobAttributes.SidesType.html#toString() */
	override public function toString():String;

}

